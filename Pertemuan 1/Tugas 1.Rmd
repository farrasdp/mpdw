...
```{r}
library(rio)
dataset = import("https://raw.githubusercontent.com/farrasdp/mpdw/main/Praktikum%201/dataset%20tugas.csv")
dataset
```
...
```{r}
ts.plot(dataset.ts, xlab="Time Period ", ylab="Returning Visit", 
...
```{r}
data_latih <- dataset[1:292,]
data_uji <- dataset[293:365,]
data_latih.ts <- ts(data_latih$Returning.Visits)
data_uji.ts <- ts(data_uji$Returning.Visits)
```
...
plot(dataset.ts, col="red",main="Plot semua data")
points(dataset.ts)

#eksplorasi data latih
plot(data_latih.ts, col="blue",main="Plot data latih")
points(data_latih.ts)

#eksplorasi data uji
plot(data_uji.ts, col="blue",main="Plot data uji")
points(data_uji.ts)
```

### Single Moving Average (SMA)

Ide dasar: data suatu periode dipengaruhi oleh data periode sebelumnya. Metode ini cocok digunakan untuk pola data stasioner atau konstan.
Prinsip dasar: data pemulusan pada periode ke-t merupakan rata rata dari m buah data pada periode ke-t hingga periode ke (t-m+1). Data pemulusan pada periode ke-t selanjutnya digunakan sebagai nilai peramalan pada periode ke t+1

Pemulusan menggunakan metode SMA dilakukan dengan fungsi `SMA()`. Dalam hal ini akan dilakukan pemulusan dengan parameter `m=4`.
```{r}
data.sma<-SMA(data_latih.ts, n=4)
data.sma
```
...
Selanjutnya akan dilakukan peramalan sejumlah data uji yaitu 73 periode. Pada metode SMA, hasil peramalan 73 periode ke depan akan bernilai sama dengan hasil peramalan 1 periode kedepan. Dalam hal ini akan dilakukan pengguabungan data aktual latih/train, data hasil pemulusan dan data hasil ramalan 73 periode kedepan.
```{r}
data.gab<-cbind(aktual=c(data_latih.ts,rep(NA,73)),pemulusan=c(data.sma,rep(NA,73)),ramalan=c(data.ramal,rep(data.ramal[length(data.ramal)],72)))
data.gab #forecast 24 periode ke depan
```

Adapun plot data deret waktu dari hasil peramalan yang dilakukan adalah sebagai berikut.
```{r}
ts.plot(dataset.ts, xlab="Time Period ", ylab="Returning Visits", main= "SMA N=4 Data Returning Visits")
points(dataset.ts)
...
error_train.sma = data_latih.ts-data.ramal[1:length(data_latih.ts)]
SSE_train.sma = sum(error_train.sma[5:length(data_latih.ts)]^2)
MSE_train.sma = mean(error_train.sma[5:length(data_latih.ts)]^2)
MAPE_train.sma = mean(abs((error_train.sma[5:length(data_latih.ts)]/data_latih.ts[5:length(data_latih.ts)])*100))
...
error_test.sma = data_uji.ts-data.gab[293:365,3]
SSE_test.sma = sum(error_test.sma^2)
MSE_test.sma = mean(error_test.sma^2)
MAPE_test.sma = mean(abs((error_test.sma/data_uji.ts*100)))
...
data.gab2 <- cbind(aktual = c(data_latih.ts,rep(NA,73)), pemulusan1 = c(data.sma,rep(NA,73)),pemulusan2 = c(data.dma, rep(NA,73)),At = c(At, rep(NA,73)), Bt = c(Bt,rep(NA,73)),ramalan = c(data.ramal2, f[-1]))
data.gab2
...
```{r}
ts.plot(dataset.ts, xlab="Time Period ", ylab="Returning Visits", main= "DMA N=4 Data Returning Visits")
points(dataset.ts)
...
```{r}
#Menghitung nilai keakuratan data latih
error_train.dma = data_latih.ts-data.ramal2[1:length(data_latih.ts)]
SSE_train.dma = sum(error_train.dma[8:length(data_latih.ts)]^2)
MSE_train.dma = mean(error_train.dma[8:length(data_latih.ts)]^2)
MAPE_train.dma = mean(abs((error_train.dma[8:length(data_latih.ts)]/data_latih.ts[8:length(data_latih.ts)])*100))
...
```{r}
#Menghitung nilai keakuratan data uji
error_test.dma = data_uji.ts-data.gab2[293:365,6]
SSE_test.dma = sum(error_test.dma^2)
MSE_test.dma = mean(error_test.dma^2)
MAPE_test.dma = mean(abs((error_test.dma/data_uji.ts*100)))
...
training<-dataset[1:292,]
testing<-dataset[293:365,]
train.ts <- ts(training$Returning.Visits)
test.ts <- ts(testing$Returning.Visits)
```
...
```{r}
#eksplorasi data
plot(dataset.ts, col="black",main="Plot semua data")
points(dataset.ts)
...
resid1<-training$Returning.Visits-ramalan1$fitted
head(resid1)
```
...
resid2<-training$Returning.Visits-ramalan2$fitted
head(resid2)
...
```{r}
selisih1<-ramalan1$mean-testing$Returning.Visits
SSEtesting1<-sum(selisih1^2)
MSEtesting1<-SSEtesting1/length(testing)

selisih2<-ramalan2$mean-testing$Returning.Visits
SSEtesting2<-sum(selisih2^2)
MSEtesting2<-SSEtesting2/length(testing)

selisihopt<-ramalanopt$mean-testing$Returning.Visits
...
```{r}
#cara lain
accuracy(ramalanopt,testing$Returning.Visits)
```
### DES
...
selisihdes1<-ramalandes1$mean-testing$Returning.Visits
selisihdes1

SSEtestingdes1<-sum(selisihdes1^2)
MSEtestingdes1<-SSEtestingdes1/length(testing$Returning.Visits)
MAPEtestingdes1<-sum(abs(selisihdes1/testing$Returning.Visits)*100)/length(testing$Returning.Visits)

selisihdes2<-ramalandes2$mean-testing$Returning.Visits
selisihdes2

SSEtestingdes2<-sum(selisihdes2^2)
MSEtestingdes2<-SSEtestingdes2/length(testing$Returning.Visits)
MAPEtestingdes2<-sum(abs(selisihdes2/testing$Returning.Visits)*100)/length(testing$Returning.Visits)

selisihdesopt<-ramalandesopt$mean-testing$Returning.Visits
selisihdesopt

SSEtestingdesopt<-sum(selisihdesopt^2)
MSEtestingdesopt<-SSEtestingdesopt/length(testing$Returning.Visits)
MAPEtestingdesopt<-sum(abs(selisihdesopt/testing$Returning.Visits)*100)/length(testing$Returning.Visits)
